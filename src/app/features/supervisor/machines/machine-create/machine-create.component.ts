...
export class MachineCreateComponent {
  ...
  onSubmit(): void {
    if (this.form.invalid) {
      ...
      this.toastService.error(this.translation.t('machines.fixRequiredFields'));
      return;
    }
    ...
    this.http.post<CatalogItem>(this.endpoint, body).subscribe({
      next: (item) => {
        ...
        this.toastService.success(this.translation.t('machines.createdSuccess', { name: item.name }));
        this.router.navigateByUrl('/supervisor/machines');
      },
      error: () => {
        this.submitting.set(false);
      },
