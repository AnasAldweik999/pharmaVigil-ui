...
export class StaffBatchesComponent {
  private readonly batchService = inject(BatchService);
  private readonly router = inject(Router);
  readonly translation = inject(TranslationService);

  private readonly productSearchUrl = `${environment.apiUrl}/api/staff/reference/products`;
  ...
  readonly gridColumns = computed<GridColumn[]>(() => {
    ...
    return [
      ...
      // Not sortable — it's computed at response-mapping time, not a real
      // persisted/joinable column, so the backend has no property to sort by.
  ...
  onGridStateChange(state: GridState): void {
    this.loadBatches(state);
  }

  onRowClick(row: unknown): void {
    this.router.navigate(['/staff/batches', (row as BatchListResponse).batchNo]);
  }

  loadBatches(state: GridState): void {
